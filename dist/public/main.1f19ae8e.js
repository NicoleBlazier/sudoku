...
document.querySelector('#toggle-dark').addEventListener('click', function () {
  ...
  localStorage.setItem('darkmode', isDarkMode); // chang mobile status bar color
...
var initSudoku = function initSudoku() {
  // clear old sudoku
  clearSudoku();
  resetBg(); // generate sudoku puzzle here

  su = sudokuGen(level);
  su_answer = _toConsumableArray(su.question);
  seconds = 0;
  saveGameInfo(); // show sudoku to div
...
var loadSudoku = function loadSudoku() {
  ...
  level_index = game.level; // show sudoku to div
  ...
  number_inputs.forEach(function (e, index) {
    e.addEventListener('click', function () {
      if (!cells[selected_cell].classList.contains('filled')) {
        ...
        cells[selected_cell].setAttribute('data-value', index + 1); // add to answer

        var row = Math.floor(selected_cell / CONSTANT.GRID_SIZE);
        var col = selected_cell % CONSTANT.GRID_SIZE;
        su_answer[row][col] = index + 1; // save game
...
init(); // for starting the confetti
